...
@section('title', 'IP Phone Materials')
    ...
    <div class="relative z-10 min-h-screen p-4 md:p-6 pb-16">
        ...
        <div class="bg-white/70 backdrop-blur-xl shadow-2xl rounded-2xl p-4 md:p-8 w-full mx-auto mt-4 md:mt-8 transition-all duration-500 transform hover:scale-[1.005]">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                <div>
                    <h1 class="text-2xl md:text-3xl font-bold text-blue-900">IP Phone Materials</h1>
                    <p class="text-yellow-600 mt-2">{{ count($ipphones) }} IP phone(s) found</p>
                </div>
                <div class="flex gap-3">
                    <a href="{{ route('superadmin.materials.ipphone.create') }}" 
                       class="btn btn-primary transform hover:scale-105 transition-transform duration-300">
                       <i class="fas fa-plus mr-2"></i>Add IP Phone
                    </a>
                    <div class="relative">
                    ...
                    <table class="min-w-full divide-y divide-blue-200/50">
                        <thead class="bg-blue-600/10 sticky top-0">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs md:text-sm font-bold text-blue-900 uppercase tracking-wider">MAC Address</th>
                        ...
                        <tbody class="bg-white/70 divide-y divide-blue-200/30" id="ipphoneTableBody">
                            @forelse($ipphones as $material)
                                <tr class="hover:bg-blue-50/50 transition-colors duration-200">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm md:text-base text-gray-800">
                                        {{ $material->materialable->mac_number ?? 'N/A' }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm md:text-base text-gray-800">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="flex space-x-3">
                                            ...
                                            <form action="{{ route('superadmin.materials.destroy', ['type' => 'ipphone', 'material' => $material->id]) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" 
                                                        class="text-red-600 hover:text-red-900 transform hover:scale-110 transition-transform"
                                                        title="Delete"
                                                        onclick="return confirm('Are you sure you want to delete this IP phone?')">
            ...
            <div class="mt-4 flex justify-between items-center text-sm text-gray-600">
                <div class="bg-blue-100/50 px-4 py-2 rounded-lg">
                    ...
                    Total: {{ count($ipphones) }} IP phone(s)
                </div>
...
<script>
    // Simple search functionality for IP phones
    document.addEventListener('DOMContentLoaded', function() {
        const searchInput = document.getElementById('ipphoneSearch');
        const tableBody = document.getElementById('ipphoneTableBody');
