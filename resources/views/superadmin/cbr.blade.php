                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                        @foreach ($carburantBranches as $branche)
                            @if($branche->site)
                            <a href="{{ route('sites.branch', ['site' => $branche->site->id, 'branchType' => 'carburant']) }}" class="block">                                    <div class="bg-gradient-to-br from-yellow-500 to-yellow-600 hover:from-yellow-600 hover:to-yellow-700 text-white rounded-xl shadow-md p-6 flex flex-col items-center justify-center h-full transition-all duration-300 transform hover:scale-105 relative overflow-hidden group">
